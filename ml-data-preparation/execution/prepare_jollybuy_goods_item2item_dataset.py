...
from src.gcsreader.config.jollybuy_config import JollybuyGoodsItem2ItemConfig
...
if __name__ == '__main__':
    ...
    reader = JollybuyGoodsItem2ItemReader(project_id, sql_context, spark_session, content_type, content_property, run_time, days, LOGGER)
    ...
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Append debug info to negative data')
    ...
    df_negative = pairs_join_and_rename(df_negative, df_candidates_dedup,
                                        ...
                                        config=JollybuyGoodsItem2ItemConfig, config_key='negative_pool',
                                        how='inner')
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Negative samples: {df_negative.count()}')
    df_negative.limit(5).show()

    ### integrate positive and negative ###
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Integrate positive and negative data')

    cols = JollybuyGoodsItem2ItemConfig.TARGET_COLS.get('all_pool', [])
    ...
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Total number of data: {df.count()}')
    df.limit(5).show()

    # join view-also-view score
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Join view also view scores')
    ...
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Join dataset `click` and `input` data with content data')
    ...
    df = pairs_join_and_rename(df, raw_data['content'],
                               ...
                               config=JollybuyGoodsItem2ItemConfig, config_key='all_pool',
                               how='inner')
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] All dataset: {df.count()}')
    df.limit(5).show()

    # parse final columns
    cols = JollybuyGoodsItem2ItemConfig.TARGET_COLS.get('final_pool', [])
    df = df.select(cols)
    df.limit(5).show()
    ...
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Positive Data: {len(df_pd.loc[df_pd["y"] == 1])}')
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Negative Data: {len(df_pd.loc[df_pd["y"] == 0])}')
    LOGGER.info(f'[Data Preparation][Jollybuy Goods Item2Item] Total Data: {len(df_pd)}')
