...
from src.gcsreader.planet_news_user2item import PlanetNewsUser2ItemReader
from src.gcsreader import dedup_by_col
from src.negative_sampler import generate_negative_samples


def get_raw_date_from_gcs():
    ...
    raw_data = {
        ...
        'user_title_embedding': gcsreader.get_user_title_embedding(),
        'user_meta': gcsreader.get_user_meta_data()
    }

    for data_name, column_to_rename_list in PlanetNewsUser2ItemConfig.COLUMN_TO_RENAME.items():
...
def join_event_with_user_profile(df_left, df_profile):
    condition = [
        df_left.openid == df_profile.openid,
        df_left.date == df_profile.date
    ]
    df = df_left.join(df_profile, on=condition, how='left') \
...
if __name__ == '__main__':
    ...
    USER_PROFILE_LIST_TO_GENERATE_PREVIOUS_DATE = ['user_category', 'user_tag', 'user_title_embedding']

    # Spark initialize
    sql_context, spark_session = initial_spark()
    gcsreader = PlanetNewsUser2ItemReader(project_id, sql_context, content_type, content_property, run_time, days=days)
    BASE_FEATURE = [
        'openid', 'uuid', 'gender', 'age', 'user_title_embedding', 'item_title_embedding', 'user_category', 'user_tag',
    ...
    # 1. get gcs data
    raw_data = get_raw_date_from_gcs()
    ...
    for key, df_profile in raw_data.items():
        if key in USER_PROFILE_LIST_TO_GENERATE_PREVIOUS_DATE:
            df_positive = join_event_with_user_profile(df_positive, df_profile)
    ...
    col2label = distinct_content_parser.parse(raw_data['item_content'],
                                              PlanetNewsUser2ItemConfig.COLS_TO_ENCODE['content'],
                                              add_ner=False)
