...
from utils import initial_spark, dump_pickle, upload_to_gcs, sample_positive_data, filter_by_interaction_count, calculate_similarity_score
...
def main():
    ...
    for profile_key, df_profile in raw_data['user_profile'].items():
        ...
        df_profile = df_profile.withColumn('date',
                                           f.udf(lambda x: (datetime.strptime(x, DAY_STRING_FORMAT) - timedelta(days=1)).strftime(DAY_STRING_FORMAT),
                                                 StringType())(f.col('date')))
    ...
    df_similar_event_pool = df_positive.withColumn('similarity_score',
                                                   f.udf(lambda x, y: calculate_similarity_score(x, y), FloatType())
                                                   (f.col('user_title_embedding'), f.col('item_title_embedding')))
    df_similar_event_pool = dedup_by_col(df_similar_event_pool, unique_col_base=['userid'], time_col='similarity_score')
    df_similar_event_pool = df_similar_event_pool.orderBy([f.col('similarity_score')], ascending=False).limit(topk_similar)
    df_similar_event_pool = df_similar_event_pool.select(['userid', 'content_id', 'publish_time'])
