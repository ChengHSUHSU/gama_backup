...
from src_v2.factory.planet_news.planet_news_item2item import PlanetNewsItem2ItemSGDFactory
from src.options.train_options import TrainSGDOptions
from src.monitoring import MailOperator, ModelMonitor
...
if __name__ == '__main__':
    ...
    # build mlflow helper
    mlflow_helper = MlflowHelper(opt, logger_name='Planet News Item2Item', logger=logger)
    ...
    logger.info('[Planet News Item2Item][MLFlow] Logging pipeline option')
    mlflow_helper.log_data(data=vars(opt), log_type='params')

    # initialize factory and training parameters
    model_factory = PlanetNewsItem2ItemSGDFactory(mode='train', **vars(opt))
    ...
    if opt.download_dataset:
        ...
        for file in file_to_download:
            logger.info(f'[Planet News Item2Item][Data Preprocessing] Download {file} from the bucket')
    ...
    logger.info('[Planet News Item2Item][Data Preprocessing] Logging dataset information')
    mlflow_helper.log_data(data={'Positive Sample Size': len(dataset[dataset['y'] == 1]),
                                 'Negative Sample Size': len(dataset[dataset['y'] == 0])},
                           log_type='metrics')

    # start training
    logger.info('[Planet News Item2Item][Model Validation] Start model validation')
    logger.info(f'[Planet News Item2Item][Model Validation] opt: {vars(opt)}')
    ...
    logger.info(f'[Planet News Item2Item][Model Validation] Show validation result \n{validation_result}')
    logger.info('[Planet News Item2Item][Model Validation] Logging validation result')
    ...
    if opt.save:
        ...
        for file_name, asset in assets_to_log.items():
            logger.info(f'[Planet News Item2Item][MLFlow] Store {file_name}')

            if file_name == model_name:
                asset.model_handler.save_model(base_path=base_path, model_path=file_name)
            else:
    ...
    if opt.monitoring:
        logger.info('[Planet News Item2Item][Model Offline Monitoring] Start offline monitoring')
        ...
        logger.info(f'[Planet News Item2Item][Model Offline Monitoring] Compare with version : {monitor.model_version}')
        ...
        baseline_factory = PlanetNewsItem2ItemSGDFactory(mode='validation',
        ...
        logger.info(f'[Planet News Item2Item][Model Validation] Baseline result \n{comparison_result}')
        logger.info('[Planet News Item2Item][Model Offline Monitoring] Compute performance improvement rate')
        ...
        logger.info(f'[Planet News Item2Item][Model Validation] Improvement Rate \n{comparison_result}')
        ...
        if len(alert_list) > 0:
            logger.info('[Planet News Item2Item][Model Offline Monitoring] Send alert mail')
            ...
            logger.info('[Planet News Item2Item][Model Offline Monitoring] No alert mail need to be sent')

        logger.info('[Planet News Item2Item][Model Offline Monitoring] Logging validation result')
