...
from src.preprocess.utils import convert_type, normalize_data, get_publish_time_to_now, process_age
from utils.logger import logger

import pandas as pd


class ClubPostUser2ItemDataPreprocesser():
    ...
    def _process_embedding(self, dataset):
        ...
        for key, feature_pair in self.configs.SEMANTICS_FEATURE.items():
            major_col = feature_pair[0]
            minor_col = feature_pair[1]

            for mode in self.configs.SEMANTICS_INTERACTION_MODE:
                self.logger.info(f'process {key} semantics: {mode}')
                dataset = process_embedding(dataset, major_col, minor_col, mode=mode)
                dataset = dataset.rename(columns={f'semantics_{mode}': f'semantics_{key}_{mode}'})
    ...
    def _aggregate_preference(self, dataset):
        ...
        self.logger.info(f'process cat0')
