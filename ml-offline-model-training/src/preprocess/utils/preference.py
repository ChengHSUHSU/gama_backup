...
class Preference():
    ...
    def _caculate_user_pref_score(self, user_data, item_data):
        """Dot product caculation of user data and item_data"""
        result = 0.0
    ...
    def extract_category_pref_score(self, df, level=['click', 'cat1'], cat_col='user_category', score_col='category_pref_score', enable_single_user=False):
        ...
        if enable_single_user:
            ...
            df[score_col] = df.apply(lambda x: self._caculate_user_pref_score(user_cat_dict, x[level[1]]), axis=1)
        else:
            df['parsed_user_cat'] = df[cat_col].apply(lambda x: self._get_pref_scores_mapping(x, level=level) if pd.notna(x) else {})
            df[score_col] = df.apply(lambda x: self._caculate_user_pref_score(x['parsed_user_cat'], x[level[1]]), axis=1)

        return df

    def extract_tag_pref_score(self, df, user_tag_col='user_tag', item_tag_col='tags', tagging_type='editor', score_col='', enable_single_user=False):
        tag_entity_list = getattr(self.config, 'TAG_ENTITY_LIST', [])   # e.g. ['person', 'location' ...]
        ...
        for tag_key in tag_entity_list:
            ...
            if enable_single_user:
                ...
                df[tagging_column_name] = df.apply(lambda x: self._caculate_user_pref_score(user_tagging_dict, x[item_tag_col]), axis=1)
            else:
                ...
                df[tagging_column_name] = df.apply(lambda x: self._caculate_user_pref_score(x[tagging_column_name], x[item_tag_col]), axis=1)
        return df
