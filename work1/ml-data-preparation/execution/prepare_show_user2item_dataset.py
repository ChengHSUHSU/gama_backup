...
from src.options.show_options import ShowUser2ItemOptions
from src.gcsreader.config.beanfun.show_user2item import ShowUser2ItemConfig
from src.gcsreader.show_user2item_aggregator import ShowUser2ItemAggregator
...
from utils import initial_spark, dump_pickle, upload_to_gcs, sample_positive_data, filter_by_interaction_count
from utils.parser import ContentDistinctValueParser
from utils.logger import logger


def get_raw_data(aggregator: ShowUser2ItemAggregator,
                 config: ShowUser2ItemConfig,
                 content_type: str,
    ...
    raw_data = {
        ...
        'statistics': df_popularity['statistics'],
        'user_category': df_user_profile['category'],
        'user_embedding': df_user_profile['title_embedding'],
        'user_meta': df_user_profile['meta']
    }
    # other data
    for content_type, profile_types_data in other_data.items():
        for _, profile_type_data in profile_types_data.items():
            profile_type_name = profile_type_data[1].replace('_title', '')
            raw_data[f'{content_type}_{profile_type_name}'] = profile_type_data[0]
    return raw_data


if __name__ == '__main__':
    config = ShowUser2ItemConfig
    opt = ShowUser2ItemOptions().parse()
    ...
    aggregator = ShowUser2ItemAggregator(project_id,
                                         sql_context,
                                         run_time,
                                         days=days,
                                         config=ShowUser2ItemConfig,
    ...
    logger.info('[Data Preparation][Show User2Item] Join positive event data with user profile data')
    ...
    df_positive = df_positive.join(raw_data['statistics'], on=['uuid', 'date', 'hour'], how='left')

    logger.info(f'[Data Preparation][SHOW User2Item] Generate negative dataset. negative_sample_size : {negative_sample_size}')
    ...
    df_negative = df_negative.join(raw_data['statistics'], on=['uuid', 'date', 'hour'], how='left')

    logger.info('[Data Preparation][SHOW User2Item] Integrate positive and negative dataset')
    ...
    logger.info('[Data Preparation][SHOW User2Item] Convert pyspark dataframe to pandas dataframe')
    ...
    col2label = distinct_content_parser.parse(raw_data['item_content'],
                                              ShowUser2ItemConfig.COLS_TO_ENCODE['content'],
                                              add_ner=False)

    logger.info('[Data Preparation][SHOW User2Item] Upload dataset to GCS')
