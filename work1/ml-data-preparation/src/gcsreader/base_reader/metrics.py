...
class MetricsReader(GcsReader):
    ...
    def _get_existed_metrics_path(self, blob_path, prefix, content_folder) -> list:
        """Private function to get the existing popularity score path
        ...
        for d in range(int(self.days)):
            ...
            for h in range(24):
                ...
                blob_prefix = f'{prefix}/{content_folder}/{current_date}/{current_hour}'

                path = blob_path.replace('INPUT_DATE', current_date).replace('INPUT_HOUR', current_hour)

                cur_path_list, _ = get_partition_path_with_blobs_check(path, blob_prefix, bucket_idx=2)
        ...
        self.logger.info(f'[Data Preparation][Metrics] Extracted metrics path: {all_path_list}')
