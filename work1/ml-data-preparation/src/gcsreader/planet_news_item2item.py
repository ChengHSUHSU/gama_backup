...
from .config.planet_configs import PlanetNewsItem2ItemConfig
from .config.base import BaseConfig


class PlanetNewsItem2ItemReader(GcsReader):

    CONFIG = {
        'planet_news': PlanetNewsItem2ItemConfig
    }
    ...
    def get_event_data(self, config_key=''):
        ...
        self.logger.info(f'[Data Preparation][Planet News Item2Item] event_path={event_path}')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] base_path={event_base_path}')
        ...
        for d in range(int(self.days)):
            ...
            path = event_path.replace('INPUT_DATE', current_date) \
                             .replace('PROPERTY', self.content_property)

            cur_path_list, _ = get_partition_path_with_blobs_check(path, prefix, bucket_idx=2)
            all_path_list.extend(cur_path_list)

        self.logger.info('[Data Preparation][Planet News Item2Item] All exist paths')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] path_list={all_path_list}')

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Get event data')
    ...
    def get_content_data(self, config_key=''):
        ...
        self.logger.info(f'[Data Preparation][Planet News Item2Item] content_path={content_path}')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] content_base_path={content_base_path}')

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Get content data')
        df_content = self.sql_context.read.parquet(content_path)

        if config_key:
            self.logger.info(f'[Data Preparation][Planet News Item2Item] Filter content data')
            df_content = filter_dataframe(df_content, config_key, self.config).filter(f.col('title_embedding').isNotNull()
                                                                                      )  # current content daily data contains data with null title_embedding
        else:
            df_content = df_content.filter(f.col('title_embedding').isNotNull())  # current content daily data contains data with null title_embedding

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Content data: {df_content.count()}')
        df_content.limit(5).show()
    ...
    def get_view_also_view_data(self, config_key=''):
        ...
        self.logger.info(f'[Data Preparation][Planet News Item2Item] view_also_view_path={view_also_view_path}')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] Get view_also_view data')
        ...
        self.logger.info('[Data Preparation][Planet News Item2Item] All exist paths')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] view_also_view path_list={all_path_list}')
        self.logger.info(f'[Data Preparation][Planet News Item2Item] view_also_view dates_list={all_dates}')
        ...
        self.logger.info(f'[Data Preparation][Planet News Item2Item] View-Also-View Data: {df_view_also_view.count()}')
        df_view_also_view.limit(5).show()

        return df_view_also_view

    def get_candidates(self, df, config_key=''):

        # get candidate from df by event

        if config_key:
            self.logger.info(f'[Data Preparation][Planet News Item2Item] Filter candidate data')
            df_candidates = filter_dataframe(df, config_key, self.config)
        else:
            df_candidates = df

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Candidates data: {df_candidates.count()}')
    ...
    def get_positive_data(self, df, config_key=''):
        ...
        if config_key:
            self.logger.info(f'[Data Preparation][Planet News Item2Item] Get all positive data')
            df_positive = filter_dataframe(df, config_key, self.config)
        else:
            df_positive = df

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Positive data: {df_positive.count()}')
    ...
    def get_negative_data(self, df_candidates_pool, df_positive_pool, neg_sample_size, major_col, candidate_col, time_col, prefix=''):
        ...
        self.logger.info(f'[Data Preparation][Planet News Item2Item] Generate negative samples from all candidates')
        df_candidates_pd = df_candidates_pool.toPandas()
        ...
        df_positive_pairs = df_positive_pool.select([major_col, prefix + candidate_col]) \
                                            .unionByName(df_positive_pool.select(f.col(prefix + candidate_col).alias(major_col),
                                                                                 f.col(major_col).alias(prefix + candidate_col)))  # union wouldn't detect schema
        df_negative = df_negative.subtract(df_positive_pairs)

        self.logger.info(f'[Data Preparation][Planet News Item2Item] Negative samples: {df_negative.count()}')
