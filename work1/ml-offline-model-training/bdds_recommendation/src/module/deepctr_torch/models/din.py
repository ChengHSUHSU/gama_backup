...
class DIN(BaseModel):
    ...
    def forward(self, X):
        ...
        deep_input_emb = torch.cat(dnn_input_emb_list, dim=-1) # torch.Size([64, 1, 40])

        # concatenate
        query_emb = torch.cat(query_emb_list, dim=-1)                     # [B, 1, E] torch.Size([64, 1, 32])
        keys_emb = torch.cat(keys_emb_list, dim=-1)                       # [B, T, E] torch.Size([64, 5, 32])
        ...
        hist = self.attention(query_emb, keys_emb, keys_length)           # [B, 1, E] torch.Size([64, 1, 32])
