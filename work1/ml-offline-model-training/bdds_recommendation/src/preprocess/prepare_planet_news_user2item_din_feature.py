...
from bdds_recommendation.src.preprocess.utils import convert_type, process_age
from bdds_recommendation.utils.logger import logger
import pandas as pd


class PlanetNewsUser2ItemDataPreprocesser():
    """The preprocesser support `xgboost` and `din` model for now
    """
    ...
    def _aggregate_preference(self, dataset):
        ...
        dataset = user_pref_helper.extract_tag_pref_score(dataset, tagging_type='editor', enable_single_user=False)

        self.logger.info(f'process tags: ner')
        dataset = user_pref_helper.extract_tag_pref_score(dataset, tagging_type='ner', enable_single_user=False)
