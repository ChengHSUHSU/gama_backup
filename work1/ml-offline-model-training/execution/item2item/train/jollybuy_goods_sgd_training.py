...
from bdds_recommendation.src.preprocess.prepare_jollybuy_item2item_xgb_feature import JollybuyGoodsItem2ItemDataPreprocesser
from bdds_recommendation.src.item2item.sgd_classifier_handler import SGDClassifierHandler
from bdds_recommendation.src.configs.jollybuy_item2item import JollybuyGoodsItem2ItemConfig
...
configs = JollybuyGoodsItem2ItemConfig()
...
if __name__ == '__main__':
    ...
    # training
    logistic_model = SGDClassifierHandler(opt, JollybuyGoodsItem2ItemConfig)
    params = {
        'loss': opt.loss,
        'penalty': opt.penalty,
        'alpha': opt.alpha,
    ...
    if opt.is_train:
        ...
        if opt.save:
            ...
            if opt.deploy:
                mlflow_helper.deploy(BASE_PATH, destination_blob_path='online_model/r12n/item2item/jollybuy_goods/sgd')
        ...
        if opt.monitoring:
            ...
            baseline_handler = SGDClassifierHandler(opt, JollybuyGoodsItem2ItemConfig)
            ...
            logger.info(f'[Jollybuy Goods Item2Item][Model Offline Monitoring] Show validation result \n{comparison_result}')
