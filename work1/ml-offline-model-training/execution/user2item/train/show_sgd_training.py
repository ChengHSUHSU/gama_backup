...
from bdds_recommendation.src_v2.factory.show.show_user2item import ShowUser2ItemLearn2RankSGDFactory
from bdds_recommendation.src.options.train_options import TrainDINOptions, TrainSGDOptions
...
if __name__ == '__main__':
    ...
    # build mlflow helper
    opt.mlflow_experiment_id = 27
    opt.mlflow_experiment_run_name = opt.dataset_blob_path.split('/')[-1]
    mlflow_helper = MlflowHelper(opt, logger_name='Show User2Item', logger=logger)
    mlflow_helper.connection()
    mlflow_helper.set_model_version(model_version=MODEL_VERSION)
    opt.experiment_name= 'test'#fb35ca912b1a4398b03aaf2eaaf0038c
    opt.verbose = 1
    ...
    logger.info('[Show User2Item][MLFlow] Logging pipeline option')
    mlflow_helper.log_data(data=vars(opt), log_type='params')

    # initialize factory and training parameters
    model_factory = ShowUser2ItemLearn2RankSGDFactory(mode='train', use_cuda=True, **vars(opt))
    ...
    if opt.download_dataset:
        ...
        for file in file_to_download:
            logger.info(f'[Show User2Item][Data Preprocessing] Download {file} from the bucket')
    ...
    #dataset = pd.DataFrame(dataset.to_dict('records')[:800])


    logger.info('[Show User2Item][Data Preprocessing] Logging dataset information')
    mlflow_helper.log_data(data={'Positive Sample Size': len(dataset[dataset['y'] == 1]),
                                 'Negative Sample Size': len(dataset[dataset['y'] == 0])},
                           log_type='metrics')

    # start training
    logger.info('[Show User2Item][Model Validation] Start model validation')
    logger.info(f'[Show User2Item][Model Validation] opt: {vars(opt)}')
    ...
    logger.info(f'[Show User2Item][Model Validation] Show validation result \n{validation_result}')
    logger.info('[Show User2Item][Model Validation] Logging validation result')
    ...
    if opt.save: 
        ...
        for file_name, asset in assets_to_log.items():
            logger.info(f'[Show User2Item][MLFlow] Store {file_name}')
    ...
    if opt.monitoring:
        logger.info('[Show User2Item][Model Offline Monitoring] Start offline monitoring')
        ...
        monitor.process()
        print('general_config.BASELINE_MODEL_PATH : ', general_config.BASELINE_MODEL_PATH)

        logger.info(f'[Show User2Item][Model Offline Monitoring] Compare with version : {monitor.model_version}')
        ...
        print('base_col2label2idx : ', base_col2label2idx.keys())
        baseline_factory = ShowUser2ItemLearn2RankDINFactory(mode='validation',
        ...
        logger.info(f'[Show User2Item][Model Validation] Baseline result \n{comparison_result}')
        logger.info('[Show User2Item][Model Offline Monitoring] Compute performance improvement rate')
        ...
        logger.info(f'[Show User2Item][Model Validation] Improvement Rate \n{comparison_result}')
        ...
        if len(alert_list) > 0:
            logger.info('[Show User2Item][Model Offline Monitoring] Send alert mail')
            ...
            logger.info('[Show User2Item][Model Offline Monitoring] No alert mail need to be sent')

        logger.info('[Show User2Item][Model Offline Monitoring] Logging validation result')
