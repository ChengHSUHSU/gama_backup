...
from bdds_recommendation.src.preprocess.utils.tag_parser import ner_parser
...
def merge_data_with_source(dataset: pd.DataFrame, source_data: pd.Series, columns: list, prefix: str = 'click') -> pd.DataFrame:
    """Function to append source data to input data

    Args:
        dataset (pd.DataFrame): input data.
        source_data (pd.Series): source content data.
            for example: 
            ```>>> source_data
                content_id      15819680
                tag             ['tag1', 'tag2']
                ...
                ...
            ```
        columns (list): columns illustrate which column to process.
        prefix (str, optional): prefix for candidate columns. Defaults to 'click'.

    Returns:
        pd.DataFrame: data with user profile data
    """

    candidate_column_to_rename_list = columns if columns else dataset.columns
    candidate_column_to_rename_map = {col: f'{prefix}{col}' for col in candidate_column_to_rename_list}
    dataset = dataset.rename(columns=candidate_column_to_rename_map)

    for source_col_name, source_col_val in source_data.items():
        if isinstance(source_col_val, list):
            dataset[source_col_name] = [source_col_val for _ in dataset.index]
        else:
            dataset[source_col_name] = source_col_val
