...
from bdds_recommendation.src_v2.configs.planet_novel.planet_novel_user2item import PlanetNovelUser2ItemDINConfig
from bdds_recommendation.src_v2.preprocess.planet_novel.planet_novel_user2item import PlanetNovelUser2ItemDINPreprocessor
...
class PlanetNovelUser2ItemLearn2RankDINFactory(CTRRankerFactory):
    ...
    def initialize(self, mode: str, col2label2idx: dict = None, **kwargs):
        ...
        self.config = PlanetNovelUser2ItemDINConfig

        if mode == 'train':
            self.preprocessor = PlanetNovelUser2ItemDINPreprocessor(configs=self.config,
            ...
            self.model_handler.load_model(kwargs.get('model_path', '/ml-model/user2item/planet_novel/din/model.latest.pickle'))
