...
from bdds_recommendation.src_v2.factory.jollybuy.jollybuy_goods_item2item import JollybuyGoodsItem2ItemLearn2RankSGDFactory
...
if __name__ == '__main__':
    ...
    # build mlflow helper
    mlflow_helper = MlflowHelper(opt, logger_name='Jollybuy Goods Item2Item', logger=logger)
    ...
    logger.info('[Jollybuy Goods Item2Item][MLFlow] Logging pipeline option')
    mlflow_helper.log_data(data=vars(opt), log_type='params')

    # initialize factory and training parameters
    model_factory = JollybuyGoodsItem2ItemLearn2RankSGDFactory(mode='train', **vars(opt))
    ...
    logger.info('[Jollybuy Goods Item2Item][Data Preprocessing] Logging dataset information')
    mlflow_helper.log_data(data={'Positive Sample Size': len(dataset[dataset['y'] == 1]),
                                 'Negative Sample Size': len(dataset[dataset['y'] == 0])},
                           log_type='metrics')

    # start training
    logger.info('[Jollybuy Goods Item2Item][Model Validation] Start model validation')
    logger.info(f'[Jollybuy Goods Item2Item][Model Validation] opt: {vars(opt)}')
    ...
    logger.info(f'[Jollybuy Goods Item2Item][Model Validation] Show validation result \n{validation_result}')
    logger.info('[Jollybuy Goods Item2Item][Model Validation] Logging validation result')
    ...
    if opt.save:
        ...
        for file_name, asset in assets_to_log.items():
            logger.info(f'[Jollybuy Goods Item2Item][MLFlow] Store {file_name}')
    ...
    if opt.monitoring:
        logger.info('[Jollybuy Goods Item2Item][Model Offline Monitoring] Start offline monitoring')
        ...
        logger.info(f'[Jollybuy Goods Item2Item][Model Offline Monitoring] Compare with version : {monitor.model_version}')
        ...
        baseline_factory = JollybuyGoodsItem2ItemLearn2RankSGDFactory(mode='validation',
        ...
        logger.info(f'[Jollybuy Goods Item2Item][Model Validation] Baseline result \n{comparison_result}')
        logger.info('[Jollybuy Goods Item2Item][Model Offline Monitoring] Compute performance improvement rate')
        ...
        logger.info(f'[Jollybuy Goods Item2Item][Model Validation] Improvement Rate \n{comparison_result}')
        ...
        if len(alert_list) > 0:
            logger.info('[Jollybuy Goods Item2Item][Model Offline Monitoring] Send alert mail')
            ...
            logger.info('[Jollybuy Goods Item2Item][Model Offline Monitoring] No alert mail need to be sent')

        logger.info('[Jollybuy Goods Item2Item][Model Offline Monitoring] Logging validation result')
