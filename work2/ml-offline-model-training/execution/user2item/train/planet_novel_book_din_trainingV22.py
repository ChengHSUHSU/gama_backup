...
from bdds_recommendation.src_v2.factory.planet_novel.planet_novel_user2item import PlanetNovelUser2ItemLearn2RankDINFactory
...
if __name__ == '__main__':
    ...
    opt.experiment_name= '024ccf71fe4a4114a43c4027e4681971'
    # build mlflow helper
    #mlflow_helper = MlflowHelper(opt, logger_name='Planet Novel User2Item', logger=logger)
    ...
    logger.info('[Planet Novel User2Item][MLFlow] Logging pipeline option')
    #mlflow_helper.log_data(data=vars(opt), log_type='params')

    # initialize factory and training parameters
    model_factory = PlanetNovelUser2ItemLearn2RankDINFactory(mode='train', use_cuda=True, **vars(opt))
    ...
    if opt.download_dataset:
        ...
        for file in file_to_download:
            logger.info(f'[Planet Novel User2Item][Data Preprocessing] Download {file} from the bucket')
    ...
    logger.info('[Planet Novel User2Item][Data Preprocessing] Logging dataset information')
    ...
    #dataset = pd.DataFrame(dataset.to_dict('records')[:1000])

    # start training
    logger.info('[Planet Novel User2Item][Model Validation] Start model validation')
    logger.info(f'[Planet Novel User2Item][Model Validation] opt: {vars(opt)}')
    ...
    print('len(dataset[dataset[y] == 1] : ', len(dataset[dataset['y'] == 1]))
    print('len(dataset[dataset[y] == 0] : ', len(dataset[dataset['y'] == 0]))
    1/0
    quit()
    ...
    if opt.save:
        ...
        for file_name, asset in assets_to_log.items():
            logger.info(f'[Planet Novel User2Item][MLFlow] Store {file_name}')
    ...
    if opt.monitoring:
        logger.info('[Planet Novel User2Item][Model Offline Monitoring] Start offline monitoring')
        ...
        logger.info(f'[Planet Novel User2Item][Model Offline Monitoring] Compare with version : {monitor.model_version}')
        ...
        baseline_factory = PlanetNovelUser2ItemLearn2RankDINFactory(mode='validation',
        ...
        logger.info(f'[Planet Novel User2Item][Model Validation] Baseline result \n{comparison_result}')
        logger.info('[Planet Novel User2Item][Model Offline Monitoring] Compute performance improvement rate')
        ...
        logger.info(f'[Planet Novel User2Item][Model Validation] Improvement Rate \n{comparison_result}')
        ...
        if len(alert_list) > 0:
            logger.info('[Planet Novel User2Item][Model Offline Monitoring] Send alert mail')
            ...
            logger.info('[Planet Novel User2Item][Model Offline Monitoring] No alert mail need to be sent')

        logger.info('[Planet Novel User2Item][Model Offline Monitoring] Logging validation result')
